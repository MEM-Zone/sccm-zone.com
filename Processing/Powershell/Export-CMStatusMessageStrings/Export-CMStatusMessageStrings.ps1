...
#>
    ...
    Process {
        ...
        Catch {
            Write-Error -Message "Could not get message. `n $_" -Category 'NotSpecified'
        }
        Finally {
...
#>
    ...
    Process {
        ...
        Catch {
            Write-Error -Message "Could not get $Severity Messages from $SourceLabel. `n $_" -Category 'NotSpecified'
        }
        Finally {
...
'@
    ...
    ForEach($Dll in $DllList) {
        ...
        #  Get dll path
        [string]$DllPathResolved = (Get-ChildItem -Path $DllPath -Recurse | Where-Object { $_.PSIsContainer -eq $false -and $_.Name -match $Dll }).FullName
        ...
        Catch {
            Write-Error -Message "Failed to export to csv file [$FilePath].`n$_" -Category 'NotSpecified'
        }
    }
}
Catch {
    Write-Error -Message "Could not get status messages from dll [$DllPath]. `n$_" -Category 'NotSpecified' -ErrorAction 'Stop'
}
Finally {
